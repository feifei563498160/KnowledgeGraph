...
# def get_log_path(log_file,prefix):
#     max1=0
#     max2=0
#     for root, sub_dirs, files in os.walk(log_file):
#         for file in files:
#             if file.startswith(prefix):
#                 print file
#                 if int(re.findall('(\d+)_(\d+)', file)[0][0])>max1:
#                     max1=int(re.findall('(\d+)_(\d+)', file)[0][0])
#                 if int(re.findall('(\d+)_(\d+)', file)[0][1])>max2:
#                     max1=int(re.findall('(\d+)_(\d+)', file)[0][1])
#     print max1,max2
#     return prefix+str('%d_%d'%(max1,max2))
# 
path_project = os.path.abspath(os.path.join(os.getcwd(), os.pardir, os.pardir))
# path_log_output=get_log_path(path_project+os.sep+"output"+os.sep,'log_regular_verify')
# if float(getsize(path_log_output))/1024/1024>2:
#     path_log_output=path_project+os.sep+"output"+os.sep+"log_regular_verify6_%d.txt"%(getsize(path_log_output)/1024/1024/10)

path_log_output=path_project+os.sep+"output"+os.sep+"log_regular_verify6_16.txt"
